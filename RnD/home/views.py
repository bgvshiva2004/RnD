...
def index(request):
    if request.method == "POST":
        ...
        Project_file_name = "".join(
            letter for letter in Project_Fellowship_No if letter.isalnum()
        )

        try:
            check = project_details.objects.get(
            ...
            years_dict = {
                "context": context,
                "years": years,
                "fellowship_no": Project_Fellowship_No,
...
def fill(request, project_id):
    try:
        ...
        new_parsed_data = {
            int(key.split("_")[1]): value for key, value in parsed_data.items()
        }
...
def ucr(request, project_id, period):
    years = period.split("-")
    ...
    data = {
        ...
        "Project_Fellowship_No": existing_project.Project_Fellowship_No,
...
def ucnr(request, project_id, period):
    years = period.split("-")
    fist_year_check = years[0]
    # print("first year",fist_year_check)
...
def get_monthewise_exp(project_id, financialYearStartYear, start_year1):
    try:
        ...
        file_path = os.path.join(
            settings.BASE_DIR,
            os.path.join("project_files", f"{project.Project_file_name}.txt"),
        )
        with open(file_path, "r") as file:
            data = json.load(file)
...
def SOE_navigation(request, project_id):
    ...
    data = {"financial_years": financial_years, "project_id": project_id}

    return render(request, "SOE_navigation.html", data)


def UCR_navigation(request, project_id):
    existing_project = project_details.objects.get(id=project_id)
    ...
    return render(request, "UCR_navigation.html", data)


def UCNR_navigation(request, project_id):
    existing_project = project_details.objects.get(id=project_id)
    ...
    return render(request, "UCNR_navigation.html", data)
...
def save_info(request, project_id):
    if request.method == "POST":
        ...
        data = json.loads(
            request.body
        )  # Assuming you are sending data using form POST method
